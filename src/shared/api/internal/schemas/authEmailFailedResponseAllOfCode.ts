/**
 * Generated by orval v6.16.0 🍺
 * Do not edit manually.
 * YouTogether API
 * This is CRUD API documentation for you-together-online project.
You can get more info about that project [here](https://github.com/youtogether-online)
 * OpenAPI spec version: 1.0.0
 */

export type AuthEmailFailedResponseAllOfCode =
  (typeof AuthEmailFailedResponseAllOfCode)[keyof typeof AuthEmailFailedResponseAllOfCode];

// eslint-disable-next-line @typescript-eslint/no-redeclare
export const AuthEmailFailedResponseAllOfCode = {
  email_already_exists: "email_already_exists",
  invalid_validation: "invalid_validation",
  code_invalid_or_expired: "code_invalid_or_expired",
  email_already_activated: "email_already_activated",
} as const;
